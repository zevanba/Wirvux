...
// Función corregida: Usamos round para evitar el redondeo al alza excesivo de ceil
function calcularNeto($presupuesto) {
    $presupuesto = floatval($presupuesto);
    $stripe_en_centimos = ($presupuesto * 100 * 0.015) + 25;
    $stripe_proyecto = round($stripe_en_centimos) / 100;
    
    $comision_proyecto_total = $presupuesto * 0.20;
    $wirvux_proyecto_limpio = $comision_proyecto_total - $stripe_proyecto;
    
    if ($wirvux_proyecto_limpio < 0) {
        return $presupuesto - $stripe_proyecto;
    } else {
        return $presupuesto * 0.80;
    }
}

// 1. Consulta para desglose mensual del AÑO ACTUAL
// Procesamos los trabajos individualmente para mantener la precisión de Stripe por proyecto
...
while ($row = mysqli_fetch_assoc($res_mensual)) {
    $mes = $row['mes'];
    $ingreso_neto = calcularNeto($row['presupuesto']);
    
    if (!isset($ingresos_por_mes[$mes])) {
        $ingresos_por_mes[$mes] = 0;
    }
    $ingresos_por_mes[$mes] += $ingreso_neto;
}
