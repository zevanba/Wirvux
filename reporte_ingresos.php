...
while ($row = mysqli_fetch_assoc($res_mensual)) {
    ...
    $ingresos_por_mes[$mes] += $ingresos_netos_proyecto;
}
...
// 2. Consulta para desglose ANUAL (Últimos 7 años)
...
while ($row_anio = mysqli_fetch_assoc($res_anual)) {
    $anio = $row_anio['anio'];
    $presupuesto_proyecto = floatval($row_anio['presupuesto']);
    
    // Tarifa real de Stripe Europa por proyecto individual
    $stripe_en_centimos = ($presupuesto_proyecto * 100 * 0.015) + 25;
    $stripe_proyecto = ceil($stripe_en_centimos) / 100;
    ...
    if (!isset($ingresos_por_anio[$anio])) {
        if (count($ingresos_por_anio) >= 7) continue; 
        $ingresos_por_anio[$anio] = 0;
    }
    $ingresos_por_anio[$anio] += $ingresos_netos_proyecto;
}
?>
...
<body>
            ...
            <table class="report-table">
                <thead>
                    <tr>
                        ...
                        <th data-key="th_total">Total Neto</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
            ...
            <table class="report-table">
                <thead>
                    <tr>
                        ...
                        <th data-key="th_total_annual">Total Anual Neto</th>
                    </tr>
                </thead>
                <tbody>
                    <?php 
                    ...
                    ?>
                        <tr><td colspan="2" class="text-center" data-key="empty_history">No hay registros históricos.</td></tr>
                    <?php endif; ?>
                </tbody>
            </table>
        </div>
    ...
    const translations = {
        'es': {
            'title_page': 'Reporte de Ingresos | Wirvux',
            ...
            'th_year': 'Año',
            'th_total_annual': 'Total Anual Neto',
        ...
        'en': {
            ...
            'th_year': 'Year',
            'th_total_annual': 'Annual Net Total',
            'empty_history': 'No historical records found.',
